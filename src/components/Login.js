...
const Login = () => {
  ...
  return (
    <div>
      ...
      <form onSubmit={(e) => e.preventDefault()} className='absolute p-12 bg-black my-36 mx-auto right-0 left-0 w-3/12 text-white bg-opacity-80'>
        <h1 className='font-bold text-3xl py-4'>{isSignInForm ? 'Sign In' : 'Sign Up'}</h1>
        {!isSignInForm && <input type='text' ref={name} placeholder='Full Name' className='p-2 my-4 bg-gray-700 w-full' />}
        <input type='text' ref={email} placeholder='Email Address' className='p-2 my-4 bg-gray-700 w-full' />
        <input type='password' ref={password} placeholder='Password' className='p-2 my-4  bg-gray-700 w-full' />
